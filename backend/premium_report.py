...
def build_comparison_pdf_bytes(meta: dict, summary: dict, compare: dict) -> bytes:
    ...
    summary: dict (from /summarize) — expects plain_summary, red_flags (list), key_numbers (dict), confidence
    compare: dict returned by /compare_offers — offer_a, offer_b, best_offer, negotiation_tips (optional)
    """
    buffer = io.BytesIO()
    ...
    subtitle_text = meta.get("generated_by", f"Generated by Car Contract AI • {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    if rf:
        # styled box
        rf_text = "<br/>".join([f"• {r}" for r in rf])
    ...
    if not all_fields:
        ...
        for f in all_fields:
            table_data.append([str(f), _safe_to_text(fieldsA.get(f, "—")), _safe_to_text(fieldsB.get(f, "—"))])
    ...
    polite = neg.get("polite", neg.get("polite_message", "—"))
    firm = neg.get("firm", neg.get("firm_message", "—"))
    legal = neg.get("legal_based", neg.get("legal_message", "—"))
...
def build_negotiation_pdf(summary: dict, score: int, reasons: list, negotiation: dict, structured: dict) -> bytes:
    ...
    elements.append(Paragraph(
        f"Generated by Car Contract AI • {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}", SMALL))
    ...
    polite = negotiation.get("polite", "—")
    firm = negotiation.get("firm", "—")
    legal = negotiation.get("legal_based", "—")

    sections = [
        ("Polite — Subject: Request to Review Lease Terms", polite),
        ("Firm — Subject: Term Revision Request", firm),
        ("Legal-Based — Subject: Formal Review of Contract Terms", legal)
    ]
