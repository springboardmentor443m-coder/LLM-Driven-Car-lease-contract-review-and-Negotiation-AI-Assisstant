...
def generate_negotiation(extracted: dict) -> dict:
    """
    Wrapper for Streamlit.
    Automatically computes fairness score and reasons.
    """

    from backend.fairness_engine import calculate_fairness

    structured = extracted.get("llm_structured_data_full") or extracted

    fairness_score, reasons = calculate_fairness(extracted)

    return generate_negotiation_messages(
