"""
Contract extractor using Groq LLM with regex fallback
"""
...
def extract_with_regex(text: str) -> Dict[str, Any]:
    """
    Lightweight heuristic extractor for contract fields
    """
...
def extract_contract(uploaded_file) -> Dict[str, Any]:
    """
    Main extraction entry point for Streamlit
    """
    ...
    try:
        ...
        # 2. Extract raw text (simple + robust)
        try:
            import pdfplumber
            text = ""
        ...
        if not text.strip():
            return {"error": "No readable text found in PDF", "raw_text": ""}
        ...
        if isinstance(llm_result, dict) and not llm_result.get("error"):
            return {
                "raw_text": text,
                "llm_structured_data_full": llm_result
            }
        ...
        return {
            "raw_text": text,
            "llm_structured_data_full": regex_result,
